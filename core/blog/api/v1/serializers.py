...
class PostSerializer(serializers.ModelSerializer):
    ...
    def get_absolute_url(self, post):
        ...
        return reverse(
            "blog:api-v1:post-detail", kwargs={"pk": post.pk}, request=request
        )
...
class CategorySerializer(serializers.ModelSerializer):
    ...
    def get_posts(self, obj):
        posts = Post.objects.filter(
            status="pub", published_date__lte=timezone.now(), category=obj
        )
    ...
    def get_absolute_url(self, category):
        request = self.context.get("request")
        return reverse(
            "blog:api-v1:category-detail", kwargs={"pk": category.pk}, request=request
        )
